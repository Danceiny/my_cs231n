...
def leaky_relu_backward(dout,cache,alpha):
    dx, x = None,cache
    dx = dout.copy()
    dx[x <= alpha * x] = 0
    return dx
...
def elu_backward(dout,cache,alpha):
    dx = dout
    x,fx = cache
    tmp = dx * (fx + alpha)
    tmp[x>0] = 0
    ...
    dx += tmp
    return dx
def maxout_backward(dout,cache,alpha):
    pass
def sigmoid_backward(dout,cache,alpha):
    pass
# /* activation backward */
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(0,F):
        db[i] = np.sum(dout[:,i,:,:])
    dw = np.zeros((F,C,HH,WW))  # np.zeros_like(w)

    for i in range(0,F):
        for j in range(0,C):
            for k in range(0,HH):
                for l in range(0,WW):
                    dw[i,j,k,l] = np.sum(dout[:,i,:,:] * x_with_pad[:,j,\
                        k : k+Hdout*stride : stride,\
                        l : l+Wdout*stride : stride])
    
    dx = np.zeros((N,C,H,W))    # np.zeros_like(x)
...
def max_pool_forward_naive(x, pool_param):
    ...
    - pool_param: dictionary with the following keys:
        ...
        - 'pool_width': The width of estride       - 'stride': The distance between adjacent pooling restride
