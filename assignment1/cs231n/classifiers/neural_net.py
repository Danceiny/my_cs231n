...
class TwoLayerNet(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, std=1e-4):
        ...
        """
        self.params = {}
        ## 标准正态分布 N(mu,sigma^2) <==> sigma * np.random.randn(...) + mu
        ## randn(dim1,dim2,...) 高维数组维数为参数
    ...
    def loss(self, X, y=None, reg=0.0, activation='ReLU'):
        ...
        # compute the class probalilities

        # scores -= np.max(scores, axis=1)[:,np.newaxis]
        # exp_scores = np.exp(scores)
        # scores[N,C] ==max==> exp_scores[N,C] 
        exp_scores = np.exp(scores - np.max(scores,axis=1,keepdims=True))
        # exp_scores[N,C] ==/==> probs[N,C]
        probs = exp_scores / np.sum(exp_scores,axis=1,keepdims=True)

        correct_logprobs = - np.log(probs[range(N),y])  # X.shape = N,D
        ...
        dscores = probs # dscores[N,C]
        dscores[range(N),y] -= 1    ## 正确分类的递减1
        dscores /= N    

        # backpropate the gredient to the parameter
        ...
        ## dscores[N,C] dot W2.T[C,H] = dhidden[N,H] ## shape same as hidden_layer
    ...
    def train(self, X, y, X_val, y_val,
        ...
        #for it in xrange(num_iters):
            ...
            #########################################################################
            dW1 = grads['W1'];dW2 = grads['W2']
            db1 = grads['b1'];db2 = grads['b2'] 
